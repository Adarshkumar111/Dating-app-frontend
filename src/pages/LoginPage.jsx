...
export default function LoginPage() {
  ...
  const onSubmit = async (e)=>{
    e.preventDefault()
    // Enforce pre-auth notice acknowledgment
    ...
    try {
      ...
      if (errorData?.needsSignup) {
        // If backend says signup not complete, send user to the next step
        // Backend does not issue signupToken here; Step1 already stores it
        // If missing, user may need to restart from Step 1
        const next = errorData.nextStep || 1
        toast.error('Please complete your signup')
        nav(`/signup/step-${next}`)
        return
      }
      if (errorData?.needsVerification) {
        ...
        const msg = errorData?.message || 'Login failed. Make sure backend is running.'
        // Avoid confusing identifier-required messages
