...
import { getChatWithUser, sendMessage as sendMsg, deleteMessage as delMsg, addReaction as addReact, uploadMedia, markAsSeen } from '../services/chatService.js'
import { useAuth } from '../context/AuthContext.jsx'
import { IoMdAdd, IoMdClose, IoMdNotifications } from 'react-icons/io'
import { MdImage, MdVideocam, MdCamera, MdPhotoLibrary, MdVideoLibrary, MdSend, MdDoneAll, MdDone } from 'react-icons/md'
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    if (hasNewMessage) {
      ...
      if (chatData) markAsSeen(chatData.chatId).catch(console.error)
    }
  }, [messages])
        ...
        {messages.map((m) => {
          ...
          return (
            <div
              ...
              className={`max-w-[70%] p-3 rounded-2xl break-words relative group ${
                m.fromSelf ? 'self-end bg-pink-400 text-white' : 'self-start bg-white border'
              }`}
            >
      ...
      <div className="p-3 border-t bg-white">
        ...
        <form onSubmit={onSend} className="flex gap-2 items-center relative">
          ...
          <input
            ...
            className="flex-1 px-4 py-2 rounded-full border focus:outline-none focus:ring-2 focus:ring-pink-400"
          />
          
          <button type="submit" disabled={!text.trim() || uploading} className="p-2 bg-pink-500 text-white rounded-full hover:bg-pink-600 disabled:opacity-50">
